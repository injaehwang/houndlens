//! `houndlens fix` — auto-generate tests, run them, optionally let AI fix failures.
//!
//! Flow:
...
pub fn run(files: Vec<String>, auto: bool, max_retries: u32) -> Result<()> {
    ...
    // Tests failed.
    if !auto {
        println!("  {} Some tests failed — run {} to let AI fix them", "!".yellow(), "houndlens fix --auto".cyan());
        println!();
        std::process::exit(1);
    }
    ...
    let adapter = houndlens_core::ai::detect_adapter();
    if adapter.is_none() {
        ...
        println!("      {} env var (any command)", "HOUNDLENS_AI_CMD".cyan());
        println!();
        std::process::exit(1);
    }
    ...
    for attempt in 1..=max_retries {
        ...
        let failures = houndlens_core::ai::parse_test_failures(&raw_output);
        ...
        let rerun_failures = houndlens_core::ai::parse_test_failures(&rerun_output);
...
fn build_fix_request(cwd: &Path, failures: Vec<houndlens_core::ai::TestFailure>) -> houndlens_core::ai::FixRequest {
    use houndlens_core::ai::FileContent;
...
fn generate_rs_tests(source_file: &str, functions: &[FnInfo]) -> (String, String) {
    ...
    let mut content = String::from("// Auto-generated by houndlens fix\n\n");
