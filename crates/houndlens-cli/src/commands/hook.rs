...
# houndlens pre-commit hook — semantic verification before commit
# Installed by: houndlens hook install
...
echo "[houndlens] Verifying staged changes..."

# Run houndlens verify on staged changes
houndlens verify --format text 2>&1
EXIT_CODE=$?

if [ $EXIT_CODE -ne 0 ]; then
    echo ""
    echo "[houndlens] ❌ Verification failed. Fix issues above or commit with --no-verify to bypass."
    exit 1
fi

echo "[houndlens] ✅ Verification passed."
exit 0
"#;

const PRE_PUSH_HOOK: &str = r#"#!/bin/sh
# houndlens pre-push hook — full verification before push
# Installed by: houndlens hook install

REMOTE="$1"
URL="$2"

echo "[houndlens] Verifying changes before push..."

# Find the merge base with the remote branch
LOCAL_SHA=$(git rev-parse HEAD)
REMOTE_SHA=$(git rev-parse @{u} 2>/dev/null || echo "HEAD~10")

houndlens --format json verify --diff "$REMOTE_SHA" > /tmp/houndlens-push-result.json 2>/dev/null
EXIT_CODE=$?

if [ $EXIT_CODE -ne 0 ]; then
    BREAKING=$(cat /tmp/houndlens-push-result.json 2>/dev/null | grep -o '"breaking":[0-9]*' | grep -o '[0-9]*')
    if [ "${BREAKING:-0}" -gt 0 ]; then
        echo "[houndlens] ❌ Push blocked: $BREAKING breaking changes detected."
        echo "[houndlens] Run 'houndlens verify --diff $REMOTE_SHA' for details."
        exit 1
    fi
fi

echo "[houndlens] ✅ Push verification passed."
exit 0
"#;
...
fn uninstall(hooks_dir: &PathBuf) -> Result<()> {
    ...
    if pre_commit.exists() && is_houndlens_hook(&pre_commit)? {
    ...
    if pre_push.exists() && is_houndlens_hook(&pre_push)? {
    ...
    if removed == 0 {
        println!("{} No houndlens hooks found.", "Info:".dimmed());
    } else {
...
fn status(hooks_dir: &PathBuf) -> Result<()> {
    ...
    for name in &["pre-commit", "pre-push"] {
        ...
        if path.exists() {
            if is_houndlens_hook(&path)? {
                println!("  {} {} (houndlens)", "✓".green(), name);
            } else {
                println!("  {} {} (other — not houndlens)", "●".yellow(), name);
            }
        } else {
...
fn is_houndlens_hook(path: &PathBuf) -> Result<bool> {
    let content = fs::read_to_string(path)?;
    Ok(content.contains("houndlens"))
}
