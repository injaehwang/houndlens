//! `houndlens check` — the ONE command developers need.
//!
...
use houndlens_core::verify::{ChangeRisk, DiffSpec, SemanticChangeKind};
...
pub fn run(files: Vec<String>, format: &OutputFormat) -> Result<()> {
    ...
    if !houndlens_dir.exists() {
        if !matches!(format, OutputFormat::Json | OutputFormat::Sarif) {
            println!("  {} First run — initializing houndlens...", "→".cyan());
        }
    ...
    for test in &result.suggested_tests {
        if let Some(node) = engine.graph.get_node(test.target) {
            ...
            file_issues.entry(file).or_default().push(Issue {
                ...
                action: Some("Write tests or run `houndlens testgen`".into()),
            });
        }
    }
