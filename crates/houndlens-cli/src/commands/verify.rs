    ...
    if !result.semantic_changes.is_empty() {
        ...
        for change in &result.semantic_changes {
            let risk_badge = match change.risk {
                ...
                houndlens_core::verify::ChangeRisk::SecuritySensitive => "SECURITY".red().bold(),
            };
    ...
    if !result.invariant_violations.is_empty() {
        ...
        for v in &result.invariant_violations {
            let severity_badge = match v.severity {
                houndlens_ir::invariant::ViolationSeverity::Error => "ERR".red().bold(),
                houndlens_ir::invariant::ViolationSeverity::Warning => "WARN".yellow(),
                houndlens_ir::invariant::ViolationSeverity::Info => "INFO".dimmed(),
            };
            println!(
                "  [{}] {}:{} — {}",
                severity_badge,
