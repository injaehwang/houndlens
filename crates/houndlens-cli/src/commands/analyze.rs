//! `houndlens` (no args) — the main experience.
//!
...
pub fn run() -> Result<()> {
    ...
    // Generate snapshot.
    let snapshot = houndlens_core::snapshot::generate(
        &engine.graph,
        duration.as_millis() as u64,
    );

    // Backup previous snapshot for diff.
    let snapshot_path = houndlens_dir.join("snapshot.json");
    let prev_path = houndlens_dir.join("snapshot.prev.json");
    if snapshot_path.exists() {
        let _ = std::fs::copy(&snapshot_path, &prev_path);
    }

    // Write full snapshot (for houndlens internal use).
    let snapshot_json = serde_json::to_string_pretty(&snapshot)?;
    std::fs::write(&snapshot_path, &snapshot_json)?;

    // Write summary (lightweight, AI reads this).
    let summary = houndlens_core::summary::generate_summary(&snapshot);
    let summary_json = serde_json::to_string_pretty(&summary)?;
    std::fs::write(houndlens_dir.join("summary.json"), &summary_json)?;

    // Write changes (if previous snapshot exists).
    if prev_path.exists() {
        if let Some(changes) = houndlens_core::summary::generate_changes(&snapshot, &prev_path) {
            let changes_json = serde_json::to_string_pretty(&changes)?;
            std::fs::write(houndlens_dir.join("changes.json"), &changes_json)?;
        } else {
            // No changes — remove stale changes.json.
            let _ = std::fs::remove_file(houndlens_dir.join("changes.json"));
        }
    }
    ...
    println!();
    println!("  Tell your AI: {}", "\"let's start houndlens\"".cyan());
    println!();
...
const AI_POINTER: &str = "When the developer mentions \"houndlens\", read `.houndlens/ai-instructions.md` and `.houndlens/summary.json`, then follow the instructions.";

fn write_ai_context(cwd: &std::path::Path) -> Result<()> {
    let houndlens_dir = cwd.join(".houndlens");

    // Full instructions go inside .houndlens/ — not in project root.
    std::fs::write(houndlens_dir.join("ai-instructions.md"), r#"# houndlens — AI Instructions

## Activation

When the developer mentions "houndlens" in any form:
1. Read `.houndlens/summary.json` — lightweight project overview with instructions.
2. If `.houndlens/changes.json` exists, read it — shows what changed since last analysis.
...
For detailed data, use houndlens commands (verify, impact, query).
...
```
houndlens verify --format json --diff HEAD
```
...
- `houndlens verify --diff HEAD` checks only changed files. Use it.
...
- Full project analysis is already done by houndlens (snapshot). You do NOT repeat it.
- Only create files if the developer explicitly asks. Otherwise modify existing files.
- If you need temporary workspace, use `.houndlens/` and clean up when done.
...
- `houndlens verify --format json --diff HEAD` — verify your changes
- `houndlens impact <file> --fn <name>` — check what depends on a function before changing it
- `houndlens query "FIND functions WHERE ..."` — search the codebase
...
if command -v houndlens > /dev/null 2>&1; then
    echo "[houndlens] Verifying changes..."

    # Rescan project.
    houndlens > /dev/null 2>&1

    # Verify changed files.
    RESULT=$(houndlens verify --format json --diff HEAD 2>/dev/null)
    BREAKING=$(echo "$RESULT" | grep -o '"breaking":[0-9]*' | grep -o '[0-9]*')

    if [ "${BREAKING:-0}" -gt 0 ]; then
        echo "[houndlens] BLOCKED: $BREAKING breaking changes detected."
        echo "[houndlens] Run: houndlens verify --diff HEAD"
        exit 1
    fi
            ...
            "PostToolUse": [
                {
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "command": "\"$CLAUDE_PROJECT_DIR\"/.claude/hooks/houndlens-verify.sh",
                            "timeout": 30
                        }
                    ]
                }
