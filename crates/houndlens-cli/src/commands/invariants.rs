...
pub fn run() -> Result<()> {
    ...
    let result = houndlens_core::invariants::discover(&engine.graph);
    ...
    for inv in &result.invariants {
        ...
        let kind_tag = match &inv.kind {
            houndlens_ir::invariant::InvariantKind::ErrorsMustBeHandled { .. } => {
                "ERROR-HANDLING".red()
            }
            houndlens_ir::invariant::InvariantKind::ConventionConstraint { .. } => {
                "CONVENTION".blue()
            }
            houndlens_ir::invariant::InvariantKind::MustPrecede { .. } => "ORDERING".yellow(),
            houndlens_ir::invariant::InvariantKind::TypeUsageConstraint { .. } => {
                "TYPE-USAGE".cyan()
            }
            _ => "OTHER".dimmed(),
        };
