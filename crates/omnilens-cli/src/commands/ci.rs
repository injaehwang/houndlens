...
pub fn run(platform_override: Option<&str>, fail_on: &str, format: &OutputFormat) -> Result<()> {
    ...
    let diff_spec = if let Some(ref base) = diff_ref {
        omnilens_core::verify::DiffSpec::GitDiff {
            base: base.clone(),
            head: "HEAD".to_string(),
        }
    } else {
        omnilens_core::verify::DiffSpec::WorkingDir
    };
...
fn print_text_summary(result: &omnilens_core::verify::VerifyResult) {
    ...
    for c in &result.semantic_changes {
        let badge = match c.risk {
            ...
            omnilens_core::verify::ChangeRisk::SecuritySensitive => "SECURITY".red(),
...
fn write_github_output(result: &omnilens_core::verify::VerifyResult) {
...
fn write_gitlab_output(result: &omnilens_core::verify::VerifyResult) {
    ...
    let _ = std::fs::write("omnilens.env", content);
}

fn format_markdown_summary(result: &omnilens_core::verify::VerifyResult) -> String {
    let status = if result.has_errors() { "❌ FAIL" } else { "✅ PASS" };
    let mut md = format!(
        "## {} omnilens verification\n\n| Metric | Value |\n|--------|-------|\n",
        status
    );
    md.push_str(&format!(
        "| Risk Score | **{:.0}%** |\n| Changes | {} |\n| Breaking | {} |\n| Warnings | {} |\n\n",
        result.risk_score * 100.0,
        result.semantic_changes.len(),
        result.semantic_changes.iter().filter(|c| matches!(c.risk, omnilens_core::verify::ChangeRisk::Breaking)).count(),
        result.warning_count(),
    ));
    md
}
