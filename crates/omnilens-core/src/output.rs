...
use omnilens_ir::invariant::ViolationSeverity;
...
pub fn to_sarif(result: &VerifyResult) -> String {
    ...
    for change in result.semantic_changes.iter() {
        let rule_id = format!("omnilens/{:?}", change.kind).to_lowercase().replace(' ', "-");
    ...
    for violation in &result.invariant_violations {
        ...
        results.push(SarifResult {
            rule_id: "omnilens/invariant-violation".into(),
            level: level.into(),
            message: SarifMessage {
    ...
    if !rules.iter().any(|r| r.id == "omnilens/invariant-violation") {
        rules.push(SarifRule {
            id: "omnilens/invariant-violation".into(),
