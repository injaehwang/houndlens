...
use houndlens_ir::invariant::{Invariant, InvariantId, InvariantKind};
            ...
            match node {
                UsirNode::Function(f) => {
                    ...
                    let is_result = match ret {
                        houndlens_ir::types::TypeRef::Resolved(
                            houndlens_ir::types::ResolvedType::Result { .. },
                        ) => true,
                        houndlens_ir::types::TypeRef::Unresolved(s) => s.starts_with("Result"),
                        _ => false,
                    };
                    if is_result {
                        Some(*id)
...
fn discover_type_usage(graph: &SemanticGraph) -> Vec<Invariant> {
    ...
    let mut type_usage: std::collections::HashMap<String, Vec<houndlens_ir::NodeId>> =
