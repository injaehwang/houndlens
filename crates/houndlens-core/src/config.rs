...
pub struct StorageConfig {
    /// Where to store the houndlens index.
...
impl Config {
    ...
    pub fn detect(project_root: &Path) -> Result<Self> {
        // Check for houndlens.toml first, then auto-detect.
        let config_path = project_root.join("houndlens.toml");
    ...
    fn auto_detect(project_root: &Path) -> Self {
        Self {
            ...
            storage: StorageConfig {
                index_dir: project_root.join(".houndlens"),
