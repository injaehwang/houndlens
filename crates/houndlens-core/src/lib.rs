//! # houndlens-core
//!
//! Core orchestration for the houndlens verification engine.
...
use houndlens_index::{ChangeKind, Indexer};
use tracing::info;

/// The main houndlens engine — entry point for all operations.
pub struct Engine {
...
impl Engine {
    /// Initialize houndlens for a project directory.
    ...
    pub fn index(&mut self) -> Result<IndexResult> {
        ...
        let link_result = houndlens_graph::linker::link(&mut self.graph);
        info!(
    ...
    pub fn impact(
        ...
        fn_name: Option<&str>,
        depth: usize,
    ) -> Result<houndlens_graph::impact::ImpactResult> {
    ...
    fn find_nodes_for_file(&self, file: &Path) -> Vec<houndlens_ir::NodeId> {
    ...
    pub fn find_target(
        ...
        file: &Path,
        fn_name: Option<&str>,
    ) -> Result<(houndlens_ir::NodeId, String)> {
        use houndlens_ir::node::UsirNode;
