...
//!
//! houndlens sends a FixRequest, AI returns a FixResponse.
...
/// Request sent from houndlens to AI.
#[derive(Debug, Serialize)]
pub struct FixRequest {
    /// What houndlens wants the AI to do.
    pub task: String,
...
pub fn detect_adapter() -> Option<Box<dyn AiAdapter>> {
    // 1. HOUNDLENS_AI_CMD env var.
    if let Ok(cmd) = std::env::var("HOUNDLENS_AI_CMD") {
        if !cmd.is_empty() {
...
impl AiAdapter for ClaudeCliAdapter {
    ...
    fn fix(&self, request: &FixRequest, cwd: &Path) -> Result<FixResponse> {
        ...
        let req_path = cwd.join(".houndlens-ai-request.json");
...
impl AiAdapter for OpenAiAdapter {
    ...
    fn fix(&self, request: &FixRequest, _cwd: &Path) -> Result<FixResponse> {
        ...
        let body = serde_json::json!({
            "model": std::env::var("HOUNDLENS_AI_MODEL").unwrap_or_else(|_| "gpt-4o".to_string()),
            "messages": [
